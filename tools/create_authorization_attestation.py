...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create a local licensed-data authorization attestation"
    )
    ...
    parser.add_argument(
        "--expected",
        required=True,
        help="JSON file containing the acceptance expected object",
    )
    ...
    expected = json.loads(
        Path(args.expected).expanduser().resolve().read_text(encoding="utf-8")
    )
