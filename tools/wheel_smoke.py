...
result = subprocess.run(
    [sys.executable, "-m", "kraken", "integrity", "audit", "--input", str(fixture), "--format", "json"],
    check=True,
    capture_output=True,
    text=True,
)
...
if "snapshot_v1" not in available_profiles():
    raise SystemExit("Packaged Kraken wheel did not include the built-in vendor plugins")
if load_risk_limits(root / "configs" / "production_risk_limits.json").max_order_notional != 10000.0:
...
try:
    LiveBrokerNotConfigured().submit(OrderRequest("TEST", "buy", 1, 100, 1, "wheel-smoke"))
except RuntimeError:
    pass
else:
