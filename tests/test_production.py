...
from kraken.execution import ExecutionGateway, JsonlExecutionAudit, LiveBrokerNotConfigured, OrderRequest, PaperBroker
from kraken.operations import create_operations_server
from kraken.production import IngestionPolicy, JsonlAlertSink, ProductionRuntime, RetryPolicy, SqliteObservationStore, observation_from_mapping, validate_ingestion_batch
...
class ProductionRuntimeTests(unittest.TestCase):
    ...
    def test_ingestion_rejects_unavailable_records(self):
        ...
        future = observations[0].__class__(observations[0].timestamp_ms, 2_000_000_000_000, observations[0].close, observations[0].volume, observations[0].realized_volatility, observations[0].liquidity)
        with self.assertRaisesRegex(ValueError, "not available"):
            validate_ingestion_batch((future,), IngestionPolicy(max_clock_skew_ms=0), now_ms=1_900_000_000_000)

    def test_runtime_retries_and_stores_immutably(self):
        observations = self.make_observations()
        with tempfile.TemporaryDirectory() as directory:
            store = SqliteObservationStore(Path(directory) / "observations.db")
            runtime = ProductionRuntime(Source(observations, failures=2), store, retry=RetryPolicy(attempts=3, base_delay_seconds=0, max_delay_seconds=0), ingestion=IngestionPolicy(max_clock_skew_ms=10_000_000_000), alert_sink=JsonlAlertSink(Path(directory) / "alerts.jsonl"))
            ...
            conflict = observations[0].__class__(observations[0].timestamp_ms, observations[0].available_at_ms, observations[0].close + 1, observations[0].volume, observations[0].realized_volatility, observations[0].liquidity)
    ...
    def test_risk_and_paper_execution_are_fail_closed(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            persistent_risk = RiskEngine(RiskLimits(1000, 2000, 5000, 500, 2, 60, 25), kill_switch, state_path=Path(directory) / "risk.json")
            persistent_risk.record_order(now)
            self.assertEqual(RiskEngine(RiskLimits(1000, 2000, 5000, 500, 2, 60, 25), kill_switch, state_path=Path(directory) / "risk.json").status(now)["orders_today"], 1)
            kill_switch.activate("manual review", "operator")
            with self.assertRaisesRegex(PermissionError, "kill_switch_active"):
                gateway.submit(OrderRequest("TEST", "buy", 1, 100, now, "order-2"), 200, 200, now_ms=now)
    ...
    def test_operations_server_has_health_and_readiness_objects(self):
        with tempfile.TemporaryDirectory() as directory:
            store = SqliteObservationStore(Path(directory) / "observations.db")
            runtime = ProductionRuntime(Source(self.make_observations()), store, ingestion=IngestionPolicy(max_clock_skew_ms=10_000_000_000))
            switch = KillSwitch(Path(directory) / "kill-switch.json")
            risk = RiskEngine(RiskLimits(1000, 2000, 5000, 500, 2, 60, 25), switch)
            gateway = ExecutionGateway(PaperBroker(), risk, JsonlExecutionAudit(Path(directory) / "execution.jsonl"))
