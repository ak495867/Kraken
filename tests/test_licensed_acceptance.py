    ...
    def test_acceptance_rejects_manifest_hash_mismatch(self):
        authorization = {
            ...
            "expected": {
                ...
                "earnings_revisions": [{"underlying": "ILLUS", "fiscal_period": "2025Q1", "minimum_events": 2}],
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(ValueError, "manifest_sha256"):
                run_licensed_data_acceptance(root / "fixtures" / "illustrative_vendor_manifest.json", path, root / "missing-calendar.json")

    def test_valid_acceptance_reports_verified_lineage(self):
        root = Path(__file__).resolve().parents[1]
        source_manifest = json.loads((root / "fixtures" / "illustrative_vendor_manifest.json").read_text(encoding="utf-8"))
        with tempfile.TemporaryDirectory() as directory:
            ...
            (workspace / "licensed_snapshot_v1.json").write_text(
                json.dumps({
                    ...
                    "equity": {"timestamp": "event_time", "available_at": "published_at", "close": "settle", "volume": "shares", "realized_volatility": "rv_20d", "liquidity": "notional_liquidity"},
                    "options": {"timestamp": "event_time", "available_at": "published_at", "expiration": "expiry", "contract": "symbol", "option_type": "right", "strike": "exercise_price", "bid": "best_bid", "ask": "best_ask", "implied_volatility": "iv", "open_interest": "open_int", "volume": "contracts", "underlying": "root"},
            ...
            for index in range(96):
                ...
                sessions.append({"session_date": session.date().isoformat(), "open_ms": open_ms, "close_ms": open_ms + 86_399_999, "is_half_day": index == 1})
            ...
            report = run_licensed_data_acceptance(manifest_path, authorization_path, calendar_path)
            self.assertTrue(report.accepted)
            self.assertEqual(report.dataset_id, "acceptance-test-dataset")
            self.assertEqual(len(report.input_sha256), 7)
            self.assertEqual(report.authorization_sha256, file_sha256(authorization_path))

    def test_acceptance_requires_explicit_authorization_file(self):
        absent = Path(__file__).resolve().parents[1] / "acceptance_data" / "authorization.json"
        with self.assertRaisesRegex(ValueError, "explicit local authorization attestation"):
            run_licensed_data_acceptance("missing-manifest.json", absent, "missing-calendar.json")
