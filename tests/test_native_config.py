...
class NativeConfigTests(unittest.TestCase):
    ...
    def test_config_drives_native_sonar_regime_and_dynamics(self):
        ...
        default_regime = _core.classify_regime_with_config(
            bars[:12], bars[12:], cutoff, default
        )
        configured_regime = _core.classify_regime_with_config(
            bars[:12], bars[12:], cutoff, configured
        )
        default_dynamics = _core.compute_marine_dynamics_with_config(
            bars, cutoff, default
        )
        configured_dynamics = _core.compute_marine_dynamics_with_config(
            bars, cutoff, configured
        )
        ...
        self.assertEqual(
            default_sonar.forecast_bands[0].sample_count,
            len(bars) - default_sonar.forecast_bands[0].horizon,
        )
        self.assertLessEqual(
            default_sonar.forecast_bands[0].lower,
            default_sonar.forecast_bands[0].center,
        )
        self.assertLessEqual(
            default_sonar.forecast_bands[0].center,
            default_sonar.forecast_bands[0].upper,
        )
        self.assertLessEqual(
            configured_sonar.forecast_bands[0].upper
            - configured_sonar.forecast_bands[0].lower,
            default_sonar.forecast_bands[0].upper
            - default_sonar.forecast_bands[0].lower,
        )
        self.assertLessEqual(configured_regime.confidence, default_regime.confidence)
        self.assertGreater(
            configured_dynamics.drag.adjusted_volatility,
            default_dynamics.drag.adjusted_volatility,
        )
