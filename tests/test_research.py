...
class WalkForwardTests(unittest.TestCase):
    def test_windows_respect_embargo_and_chronology(self):
        ...
        self.assertEqual(
            report.manifest.decision_cutoff_ms, report.windows[-1].decision_cutoff_ms
        )
        for window in report.windows:
            ...
            self.assertIn(
                "Survivorship control was not declared",
                " ".join(window.integrity.warnings),
            )
