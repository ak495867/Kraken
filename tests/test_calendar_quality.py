...
EQUITY_FIXTURE = Path(__file__).resolve().parents[1] / "fixtures" / "illustrative_market_data.csv"
...
def _sessions(observations):
    return tuple(
        ExchangeSession(
            session_date=datetime.fromtimestamp(item.timestamp_ms / 1000, tz=timezone.utc).date().isoformat(),
...
class CalendarQualityTests(unittest.TestCase):
    ...
    def test_explicit_calendar_accepts_complete_local_history(self):
        report = validate_market_data_quality(self.equity[:38], self.options, self.cutoff, self.sessions)
    ...
    def test_quality_report_detects_duplicate_stale_and_incomplete_snapshots(self):
        stale = replace(self.equity[0], available_at_ms=self.equity[0].timestamp_ms + 600_000)
        ...
        self.assertTrue({"duplicate_observation_snapshot", "stale_observation_snapshot", "incomplete_option_chain", "halted_underlying_quote"}.issubset(codes))

    def test_backtest_rejects_incomplete_option_chain_under_strict_policy(self):
        calls_only = tuple(item for item in self.options if item.option_type == "call")
        with self.assertRaisesRegex(ValueError, "Calendar and market-data quality validation failed"):
