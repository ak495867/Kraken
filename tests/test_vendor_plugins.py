...
from kraken.normalizers import (
    discover_vendor_plugins,
    load_vendor_plugin,
...
class VendorPluginTests(unittest.TestCase):
    def test_builtin_plugins_are_versioned_and_provenanced(self):
        ...
        self.assertEqual(
            {(item.provider, item.version) for item in plugins},
            {("canonical", "1.0.0"), ("snapshot_v1", "1.0.0")},
        )
                ...
                json.dumps(
                    {
                        "provider": "provider",
                        "version": "1.0.0",
                        "schema": "kraken_vendor_mapping_plugin/v1",
                        "mapping": load_vendor_plugin("canonical").mapping,
                    }
                ),
                encoding="utf-8",
            )
