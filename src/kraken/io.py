...
def parse_timestamp(value: Any) -> int:
    if isinstance(value, bool):
        raise ValueError(
            "Timestamp values must be UTC ISO-8601 strings or Unix milliseconds"
        )
    if isinstance(value, (int, float)):
        if not math.isfinite(float(value)) or int(value) <= 0:
            raise ValueError(
                "Unix millisecond timestamps must be positive finite values"
            )
        return int(value)
...
def observation_from_mapping(row: dict[str, Any], row_number: int) -> Observation:
    ...
    observation = Observation(
        ...
        realized_volatility=_parse_number(
            row["realized_volatility"], "realized_volatility"
        ),
    ...
    if (
        ...
        raise ValueError(
            f"Row {row_number} has a negative volume, realized_volatility, or liquidity"
        )
...
def _read_json(path: Path) -> list[dict[str, Any]]:
    ...
    if not isinstance(payload, list):
        raise ValueError(
            "JSON input must be an array or an object with an observations array"
        )
...
def load_observations(path: str | Path) -> tuple[Observation, ...]:
    ...
    observations = tuple(
        observation_from_mapping(row, index + 2) for index, row in enumerate(rows)
    )
    if any(
...
def option_quote_from_mapping(row: dict[str, Any], row_number: int) -> OptionQuote:
    ...
    if missing:
        raise ValueError(
            f"Option row {row_number} is missing required fields: {', '.join(missing)}"
        )
    ...
    quote = OptionQuote(
        ...
        implied_volatility=_parse_number(
    ...
    if quote.available_at_ms < quote.timestamp_ms:
        raise ValueError(
            f"Option row {row_number} has available_at earlier than timestamp"
        )
    ...
    if (
        ...
        or quote.implied_volatility < 0.0
        or quote.open_interest < 0.0
        or quote.volume < 0.0
    ):
...
def load_option_quotes(path: str | Path) -> tuple[OptionQuote, ...]:
    ...
    quotes = tuple(
        option_quote_from_mapping(row, index + 2) for index, row in enumerate(rows)
    )
    if any(
        quotes[index].timestamp_ms < quotes[index - 1].timestamp_ms
...
def corporate_action_from_mapping(
    row: dict[str, Any], row_number: int
) -> CorporateAction:
    missing = [
        field
        for field in CORPORATE_ACTION_REQUIRED_FIELDS
        if field not in row or row[field] in (None, "")
    ]
    if missing:
        raise ValueError(
            f"Corporate-action row {row_number} is missing required fields: {', '.join(missing)}"
        )
    ...
    if action.action_type not in {"stock_split", "reverse_split"}:
        raise ValueError(
            f"Corporate-action row {row_number} action_type must be stock_split or reverse_split"
        )
...
def option_adjustment_from_mapping(
    row: dict[str, Any], row_number: int
) -> OptionContractAdjustment:
    missing = [
        field
        for field in OPTION_ADJUSTMENT_REQUIRED_FIELDS
        if field not in row or row[field] in (None, "")
    ]
    if missing:
        raise ValueError(
            f"Option-adjustment row {row_number} is missing required fields: {', '.join(missing)}"
        )
    ...
    if (
        not adjustment.underlying
        or not adjustment.pre_contract
        or not adjustment.post_contract
    ):
...
def load_corporate_actions(path: str | Path) -> tuple[CorporateAction, ...]:
    ...
    actions = tuple(
        corporate_action_from_mapping(row, index + 2) for index, row in enumerate(rows)
    )
    if any(
        actions[index].effective_at_ms < actions[index - 1].effective_at_ms
...
def load_option_adjustments(path: str | Path) -> tuple[OptionContractAdjustment, ...]:
    ...
    adjustments = tuple(
        option_adjustment_from_mapping(row, index + 2) for index, row in enumerate(rows)
    )
    if any(
        adjustments[index].effective_at_ms < adjustments[index - 1].effective_at_ms
        for index in range(1, len(adjustments))
    ):
        raise ValueError(
            "Option-adjustment effective timestamps must be non-decreasing"
        )
    return adjustments
...
def distribution_from_mapping(
    ...
    if missing:
        raise ValueError(
            f"Distribution row {row_number} is missing required fields: {', '.join(missing)}"
        )
    ...
    if distribution.distribution_type not in {"cash_dividend", "special_distribution"}:
        raise ValueError(
            f"Distribution row {row_number} distribution_type must be cash_dividend or special_distribution"
        )
...
def load_distributions(path: str | Path) -> tuple[DistributionRecord, ...]:
    ...
    distributions = tuple(
        distribution_from_mapping(row, index + 2) for index, row in enumerate(rows)
    )
    if any(
        distributions[index].effective_at_ms < distributions[index - 1].effective_at_ms
...
def earnings_event_from_mapping(row: dict[str, Any], row_number: int) -> EarningsEvent:
    missing = [
        field
        for field in EARNINGS_REQUIRED_FIELDS
        if field not in row or row[field] in (None, "")
    ]
    if missing:
        raise ValueError(
            f"Earnings row {row_number} is missing required fields: {', '.join(missing)}"
        )
    event = EarningsEvent(
    ...
    if not event.underlying or not event.fiscal_period:
        raise ValueError(
            f"Earnings row {row_number} has an empty underlying or fiscal_period"
        )
...
def load_earnings_events(path: str | Path) -> tuple[EarningsEvent, ...]:
    ...
    events = tuple(
        earnings_event_from_mapping(row, index + 2) for index, row in enumerate(rows)
    )
    if any(
        events[index].event_at_ms < events[index - 1].event_at_ms
...
def load_vendor_manifest(path: str | Path) -> LicensedVendorManifest:
    ...
    if missing:
        raise ValueError(
            f"Vendor manifest is missing required fields: {', '.join(missing)}"
        )
    ...
    manifest = LicensedVendorManifest(
        ...
        option_adjustments_path=resolve_reference(
            payload.get("option_adjustments_path")
        ),
        ...
        sector=(
            str(payload["sector"]).strip()
            if payload.get("sector") not in (None, "")
            else None
        ),
        underlying_universe=(
            str(payload["underlying_universe"]).strip()
            if payload.get("underlying_universe") not in (None, "")
            else None
        ),
        normalizer_profile=(
            str(payload["normalizer_profile"]).strip()
            if payload.get("normalizer_profile") not in (None, "")
            else None
        ),
