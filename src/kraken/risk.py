...
class RiskLimits:
    ...
    def from_mapping(cls, payload: dict[str, Any]) -> "RiskLimits":
        ...
        if set(payload) != required:
            raise ValueError(
                "Risk policy keys must exactly match the declared risk-limit contract"
            )
        limits = cls(
            float(payload["max_order_notional"]),
            float(payload["max_position_notional"]),
            float(payload["max_gross_notional"]),
            float(payload["max_daily_loss"]),
            int(payload["max_orders_per_day"]),
            float(payload["max_data_age_seconds"]),
            float(payload["max_slippage_bps"]),
        )
        limits.validate()
        return limits
...
class KillSwitch:
    ...
    def state(self) -> KillSwitchState:
        ...
        return KillSwitchState(
            ...
            int(payload.get("changed_at_ms", 0)),
        )
    ...
    def activate(self, reason: str, actor: str) -> KillSwitchState:
        ...
        state = KillSwitchState(
            True, reason.strip(), actor.strip(), int(time.time() * 1000)
        )
        self._write(state)
        return state
...
class RiskEngine:
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            self.orders_by_day = {
                str(key): int(value)
                for key, value in payload.get("orders_by_day", {}).items()
            }
            self.daily_pnl = {
                str(key): float(value)
                for key, value in payload.get("daily_pnl", {}).items()
            }
    ...
    def evaluate_order(
        ...
        if evaluated_at - quote_timestamp_ms > int(
            self.limits.max_data_age_seconds * 1000
        ):
    ...
    def status(self, now_ms: int | None = None) -> dict[str, Any]:
        ...
        with self.lock:
            return {
                ...
                "orders_today": self.orders_by_day.get(day, 0),
                "daily_pnl": self.daily_pnl.get(day, 0.0),
                "limits": self.limits,
            }
