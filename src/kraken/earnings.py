...
def validate_earnings_event_integrity(
    ...
    for event in known:
        ...
        if identity in seen:
            issues.append(
                IntegrityIssue(
                    "duplicate_earnings_event",
                    "error",
                    "Earnings events must be unique by event time, underlying, fiscal period, and type",
                )
            )
            continue
        seen.add(identity)
        if event.available_at_ms > decision_cutoff_ms:
            issues.append(
                IntegrityIssue(
                    "earnings_availability_leakage",
                    "error",
                    "Earnings event was not available at the decision cutoff",
                )
            )
    if not known:
