...
class PaperBroker:
    ...
    def submit(self, request: OrderRequest) -> Fill:
        ...
        execution_price = request.reference_price * (
            1.0 + direction * self.slippage_bps / 10000.0
        )
        fill = Fill(
            str(uuid.uuid4()),
            "filled",
        ...
        with self.lock:
            ...
            self.positions[request.symbol] = (
                self.positions.get(request.symbol, 0.0) + direction * request.quantity
            )
            self._persist()
        return fill
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            self.positions = {
                ...
                for symbol, quantity in payload.get("positions", {}).items()
            }
    ...
    def _persist(self) -> None:
        ...
        temporary.write_text(
            json.dumps({"positions": self.positions}, sort_keys=True) + "\n",
            encoding="utf-8",
        )
...
class LiveBrokerNotConfigured:
    def submit(self, request: OrderRequest) -> Fill:
        raise RuntimeError(
            "Live broker adapter is not configured; use a provider-specific implementation after independent certification"
        )
...
class HttpJsonBroker:
    ...
    def submit(self, request: OrderRequest) -> Fill:
        ...
        if not isinstance(result, dict) or result.get("status") not in {
            ...
            raise RuntimeError(
                "Broker response did not confirm an accepted or filled order"
            )
        return Fill(
            str(result["order_id"]),
            str(result["status"]),
            request.symbol,
            request.side.lower(),
            float(result.get("quantity", request.quantity)),
            float(result["execution_price"]),
            int(result.get("filled_at_ms", time.time() * 1000)),
        )
    ...
    def completed(self) -> dict[str, Fill]:
        ...
        try:
            ...
            for record in records:
                ...
                fill = record["fill"]
                completed[str(request["idempotency_key"])] = Fill(
                    str(fill["order_id"]),
                    str(fill["status"]),
                    str(fill["symbol"]),
                    str(fill["side"]),
                    float(fill["quantity"]),
                    float(fill["execution_price"]),
                    int(fill["filled_at_ms"]),
                    str(fill.get("reason", "")),
                )
            return completed
...
class ExecutionGateway:
    def __init__(
        ...
        if mode == "live" and (
            not allow_live
            or os.environ.get("KRAKEN_LIVE_TRADING_CONFIRMATION")
            != "I_UNDERSTAND_LIVE_TRADING_RISK"
        ):
            raise RuntimeError(
                "Live execution requires explicit configuration and confirmation"
            )
        self.broker = broker
        self.risk = risk
        self.audit = audit
    ...
    def submit(
        ...
        if not decision.approved:
            ...
            raise PermissionError(
                "Order rejected by risk controls: " + ",".join(decision.reasons)
            )
        fill = self.broker.submit(request)
        slippage_bps = (
            abs(fill.execution_price / request.reference_price - 1.0) * 10000.0
        )
    ...
    def status(self) -> dict[str, Any]:
        with self.lock:
            return {
                "mode": self.mode,
                "completed_orders": len(self.completed),
                "live_enabled": self.mode == "live" and self.allow_live,
            }
