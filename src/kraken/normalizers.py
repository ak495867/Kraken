...
def _validate_mapping(mapping: object) -> dict[str, dict[str, str]]:
    ...
    for label, canonical_fields in CANONICAL_MAPPING.items():
        ...
        if not isinstance(fields, dict) or set(fields) != set(canonical_fields):
            raise ValueError(
                f"Vendor mapping {label} fields must exactly match Kraken’s canonical {label} schema"
            )
        if not all(
            isinstance(source_name, str) and source_name.strip()
            for source_name in fields.values()
        ):
            raise ValueError(
                f"Vendor mapping {label} source fields must be non-empty strings"
            )
        validated[label] = {
            str(target): str(source) for target, source in fields.items()
        }
    return validated
...
def discover_vendor_plugins(
    plugin_root: str | Path | None = None,
) -> tuple[VendorMappingPlugin, ...]:
    root = (
        Path(plugin_root).expanduser().resolve()
        if plugin_root is not None
        else _default_plugin_root()
    )
    ...
    for manifest_path in _plugin_manifest_paths(root):
        ...
        if not isinstance(payload, dict):
            raise ValueError(
                f"Vendor plugin manifest must be an object: {manifest_path}"
            )
        ...
        if (
            ...
            or schema != "kraken_vendor_mapping_plugin/v1"
        ):
        ...
        if not contract_path.is_file():
            raise ValueError(
                f"Vendor plugin fixture contract is missing: {contract_path}"
            )
        ...
        if (
            ...
            raise ValueError(
                f"Vendor plugin fixture contract does not match manifest: {manifest_path}"
            )
        mapping = _validate_mapping(payload.get("mapping"))
        for label in CANONICAL_MAPPING:
            field_name = (
                "required_option_source_fields"
                if label == "options"
                else "required_equity_source_fields"
            )
            ...
            if (
                ...
                raise ValueError(
                    f"Vendor plugin fixture contract fields do not match manifest: {manifest_path}"
                )
        key = (provider, version)
        if key in seen:
            raise ValueError(
                f"Duplicate vendor plugin provider and version: {provider} {version}"
            )
        seen.add(key)
        plugins.append(
            VendorMappingPlugin(
...
def load_vendor_plugin(
    profile: str, version: str | None = None, plugin_root: str | Path | None = None
) -> VendorMappingPlugin:
    normalized_provider = profile.strip().lower()
    candidates = [
        item
        for item in discover_vendor_plugins(plugin_root)
    ...
    if not candidates:
        ...
        raise ValueError(
            f"Unsupported normalizer profile: {profile}. Available profiles: {available}"
        )
...
def available_profiles(plugin_root: str | Path | None = None) -> tuple[str, ...]:
    return tuple(
        sorted({item.provider for item in discover_vendor_plugins(plugin_root)})
    )
...
def _canonical_rows(
    rows: list[dict[str, str]], mapping: dict[str, str], label: str
) -> list[dict[str, str]]:
    if not rows:
    ...
    if missing:
        raise ValueError(
            f"{label} export is missing mapped source fields: {', '.join(missing)}"
        )
    ...
    for row_number, row in enumerate(rows, 2):
        ...
        if missing:
            raise ValueError(
                f"{label} export row {row_number} is missing mapped source fields: {', '.join(missing)}"
            )
...
def normalize_provider_exports(
    ...
    equity_rows = _canonical_rows(
        _read_csv(Path(equity_source).expanduser().resolve()),
        mapping["equity"],
        "Equity",
    )
    option_rows = _canonical_rows(
        _read_csv(Path(options_source).expanduser().resolve()),
        mapping["options"],
        "Option",
    )
        ...
        configuration_fingerprint=configuration_fingerprint(
            {
                ...
                "mapping_file": (
                    str(Path(mapping_file).expanduser().resolve())
                    if mapping_file is not None
                    else None
                ),
