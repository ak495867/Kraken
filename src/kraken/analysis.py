...
from .models import DistanceCalibrationReport, Echo, ForecastBand, Observation, RegimeRiskReport, SonarReport
...
def _sonar_report(native: _core.SonarResult, cutoff: int, audit) -> SonarReport:
    return SonarReport(
        ...
        echoes=tuple(Echo(item.horizon, item.displacement, item.strength, item.anomaly_score) for item in native.echoes),
        forecast_bands=tuple(ForecastBand(item.horizon, item.lower, item.center, item.upper, item.coverage, item.sample_count) for item in native.forecast_bands),
        integrity=audit,
    )
...
def run_sonar_tracker(
    ...
    native = _core.compute_sonar_with_config(as_native_bars(eligible_observations(provided, decision_cutoff_ms)), decision_cutoff_ms, config)
    report = _sonar_report(native, decision_cutoff_ms, audit)
    return replace(report, manifest=create_run_manifest({"analysis": "sonar_tracker", "research_config": canonical_config}, {"observations": provided}, report, decision_cutoff_ms, warnings=report.integrity.warnings))


def _split_reference_evaluation(observations: tuple[Observation, ...], reference_size: int | None, minimum_reference: int, minimum_evaluation: int) -> tuple[tuple[Observation, ...], tuple[Observation, ...]]:
    if len(observations) < minimum_reference + minimum_evaluation:
        raise ValueError("Regime analysis has insufficient eligible observations for the configured reference and evaluation windows")
    split = reference_size if reference_size is not None else max(minimum_reference, len(observations) * 2 // 3)
    if split < minimum_reference or len(observations) - split < minimum_evaluation:
        raise ValueError("reference_size must preserve the configured reference and evaluation minimums")
...
def analyze_regime_risk(
    ...
    preflight_audit = build_integrity_audit(
        provided,
        decision_cutoff_ms,
        {"analysis": "regime_risk", "research_config": canonical_config, "reference_size": reference_size},
    ...
    reference, evaluation = _split_reference_evaluation(eligible, reference_size, config.reference_minimum_points, evaluation_minimum)
    audit = build_integrity_audit(
        provided,
        decision_cutoff_ms,
        {"analysis": "regime_risk", "research_config": canonical_config, "reference_size": len(reference), "evaluation_size": len(evaluation)},
        minimum_points=minimum_points,
        universe_id=universe_id,
        survivorship_controlled=survivorship_controlled,
    )
    native = _core.classify_regime_with_config(as_native_bars(reference), as_native_bars(evaluation), decision_cutoff_ms, config)
    ...
    return replace(report, manifest=create_run_manifest({"analysis": "regime_risk", "research_config": canonical_config, "reference_size": len(reference)}, {"observations": provided}, report, decision_cutoff_ms, warnings=report.integrity.warnings))
