...
def calibration_diagnostics(
    ...
    if not grouped:
        raise ValueError(
            "At least one backtest report is required for calibration diagnostics"
        )
    ...
    for label in sorted(grouped):
        ...
        groups.append(
            CalibrationDiagnosticGroup(
                group_label=label,
                window_count=count,
                implied_move_coverage_rate=sum(
                    1 for window in windows if window.implied_move_covered
                )
                / count,
                mean_absolute_move_ratio=sum(
                    window.absolute_move_ratio for window in windows
                )
                / count,
                mean_composite_dynamics_risk=sum(
                    window.composite_dynamics_risk for window in windows
                )
                / count,
                mean_drag_adjusted_volatility=sum(
                    window.drag_adjusted_volatility for window in windows
                )
                / count,
                mean_tidal_current_strength=sum(
                    window.tidal_current_strength for window in windows
                )
                / count,
                mean_cavitation_score=sum(window.cavitation_score for window in windows)
                / count,
                mean_buoyancy_score=sum(window.buoyancy_score for window in windows)
                / count,
            )
        )
...
def time_sliced_calibration_diagnostics(
    ...
    if not windows:
        raise ValueError(
            "At least one backtest window is required for time-sliced diagnostics"
        )
    ...
    for start in range(0, len(windows), slice_size):
        ...
        slices.append(
            CalibrationTimeSlice(
                ...
                window_count=count,
                implied_move_coverage_rate=sum(
                    1 for window in subset if window.implied_move_covered
                )
                / count,
                mean_absolute_move_ratio=sum(
                    window.absolute_move_ratio for window in subset
                )
                / count,
                mean_composite_dynamics_risk=mean_risk,
                mean_drag_adjusted_volatility=sum(
                    window.drag_adjusted_volatility for window in subset
                )
                / count,
                mean_tidal_current_strength=sum(
                    window.tidal_current_strength for window in subset
                )
                / count,
                mean_cavitation_score=sum(window.cavitation_score for window in subset)
                / count,
                mean_buoyancy_score=sum(window.buoyancy_score for window in subset)
                / count,
                composite_risk_change_from_prior_slice=(
                    None if previous_risk is None else mean_risk - previous_risk
                ),
            )
        )
...
def load_backtest_report(path: str | Path) -> EquityOptionsBacktestReport:
    ...
    if not isinstance(payload, dict) or (set(payload) - {"manifest"}) != required:
        raise ValueError(
            "Backtest diagnostics input must be a Kraken equity-options JSON report"
        )
    from .models import (
    ...
    for item in payload["windows"]:
        ...
        integrity = IntegrityAudit(
            ...
            excluded_future_timestamp_count=integrity_payload[
            ...
            issues=tuple(
                IntegrityIssue(**issue) for issue in integrity_payload["issues"]
            ),
    ...
    if manifest_payload is not None:
        ...
        if (
            ...
            or set(manifest_payload) != manifest_fields
        ):
            raise ValueError(
                "Backtest report manifest fields do not match the immutable Kraken manifest schema"
            )
        manifest = RunManifest(
            ...
            input_sha256=tuple(
                ...
                for item in manifest_payload["input_sha256"]
            ),
