...
def _load_authorization(path: str | Path) -> dict[str, Any]:
    ...
    if not source.is_file():
        raise ValueError(
            "Licensed acceptance data requires an explicit local authorization attestation file"
        )
    ...
    if loaded.get("license_accepted") is not True:
        raise ValueError(
            "Licensed acceptance authorization must set license_accepted to true"
        )
    if (
        not isinstance(loaded.get("authorized_by"), str)
        or not loaded["authorized_by"].strip()
    ):
        raise ValueError(
            "Licensed acceptance authorization must identify authorized_by"
        )
    if (
        not isinstance(loaded.get("dataset_id"), str)
        or not loaded["dataset_id"].strip()
    ):
        raise ValueError("Licensed acceptance authorization must identify dataset_id")
    if not isinstance(loaded.get("expected"), dict):
        raise ValueError(
            "Licensed acceptance authorization must include an expected object"
        )
    if (
        not isinstance(loaded.get("manifest_sha256"), str)
        or len(loaded["manifest_sha256"]) != 64
    ):
        raise ValueError(
            "Licensed acceptance authorization must include manifest_sha256"
        )
    if not isinstance(loaded.get("files"), dict) or not loaded["files"]:
        raise ValueError(
            "Licensed acceptance authorization must include a files hash object"
        )
    if (
        not isinstance(loaded.get("authorized_at"), str)
        or not loaded["authorized_at"].strip()
    ):
        raise ValueError("Licensed acceptance authorization must include authorized_at")
    if (
        not isinstance(loaded.get("license_name"), str)
        or not loaded["license_name"].strip()
    ):
        raise ValueError("Licensed acceptance authorization must include license_name")
    if (
        not isinstance(loaded.get("expires_at"), str)
        or not loaded["expires_at"].strip()
    ):
    ...
    if expires_at_ms <= authorized_at_ms:
        raise ValueError(
            "Licensed acceptance authorization expires before it was issued"
        )
...
def _verified_input_hashes(
    ...
    if (
        not _is_sha256(authorization["manifest_sha256"])
        or authorization["manifest_sha256"] != manifest_hash
    ):
        raise ValueError(
            "Licensed acceptance manifest_sha256 does not match the supplied manifest"
        )
    expected_paths = {
    ...
    expected_paths = {
        key: path for key, path in expected_paths.items() if path is not None
    }
    attested_files = authorization["files"]
    if set(attested_files) != set(expected_paths):
        raise ValueError(
            "Licensed acceptance files must exactly cover every manifest input"
        )
...
def _required_int(expected: dict[str, Any], name: str) -> int:
    ...
    if not isinstance(value, int) or value < 1:
        raise ValueError(
            f"Licensed acceptance expected.{name} must be an integer of at least one"
        )
    return value
...
def run_licensed_data_acceptance(
    ...
    if (
        not isinstance(missing_contracts, list)
        or not missing_contracts
        or not all(isinstance(item, str) and item for item in missing_contracts)
    ):
        raise ValueError(
            "Licensed acceptance expected.missing_contracts must list at least one contract intentionally absent from the authorized sample"
        )
    if not isinstance(earnings_revisions, list) or not earnings_revisions:
        raise ValueError(
            "Licensed acceptance expected.earnings_revisions must list at least one authorized revision expectation"
        )
    if (
        not isinstance(half_day_dates, list)
        or not half_day_dates
        or not all(isinstance(item, str) and item for item in half_day_dates)
    ):
        raise ValueError(
            "Licensed acceptance expected.half_day_dates must list at least one declared half-day session"
        )
    dataset = FilePointInTimeVendorAdapter(manifest_path).load()
    authorization_sha256, manifest_sha256, input_sha256 = _verified_input_hashes(
        manifest_path, authorization_path, authorization, dataset
    )
    if authorization["dataset_id"] != dataset.manifest.dataset_id:
        raise ValueError(
            "Licensed acceptance authorization dataset_id does not match the vendor manifest"
        )
    if len(dataset.corporate_actions) < required_actions:
        raise ValueError(
            "Authorized acceptance sample lacks the declared minimum corporate-action cases"
        )
    if len(dataset.option_adjustments) < required_adjustments:
        raise ValueError(
            "Authorized acceptance sample lacks the declared minimum option-adjustment cases"
        )
    ...
    if not set(half_day_dates).issubset(declared_half_days):
        raise ValueError(
            "Authorized acceptance calendar does not contain every declared half-day session"
        )
    ...
    if not corporate.valid:
        ...
        raise ValueError(
            f"Authorized corporate-action acceptance check failed: {details}"
        )
    distributions = validate_distribution_integrity(
        dataset.equity, dataset.distributions, cutoff, dataset.manifest.price_basis
    )
    ...
    if not earnings.valid:
        ...
        raise ValueError(
            f"Authorized earnings availability acceptance check failed: {details}"
        )
    revision_counts = Counter(
        (item.underlying, item.fiscal_period) for item in dataset.earnings_events
    )
    for expectation in earnings_revisions:
        ...
        if (
            ...
            or not isinstance(fiscal_period, str)
            or not isinstance(minimum_events, int)
            or minimum_events < 2
        ):
            raise ValueError(
                "Each earnings revision expectation requires underlying, fiscal_period, and minimum_events of at least two"
            )
        if revision_counts[(underlying, fiscal_period)] < minimum_events:
            raise ValueError(
                f"Authorized acceptance sample lacks declared earnings revisions for {underlying} {fiscal_period}"
            )
    ...
    if present_missing:
        raise ValueError(
            f"Contracts declared missing are present in the authorized sample: {', '.join(present_missing)}"
        )
    ...
    if not quality.valid:
        ...
        raise ValueError(
            f"Authorized calendar and missing-data acceptance check failed: {details}"
        )
    verified_checks = (
