...
def render_time_sliced_diagnostics(
    ...
    if not report.slices:
        raise ValueError(
            "Time-sliced diagnostics must contain at least one slice to render"
        )
    ...
    axes[0].plot(
        ...
        linewidth=1.8,
        marker="o",
        label="Buoyancy score",
    )
    axes[0].set_ylim(0.0, 1.0)
    axes[0].set_ylabel("Bounded score", color="#c9d8e6")
    axes[0].set_title(
        "Kraken Marine Dynamics: Chronological Regime-Shift Slices",
        color="#f4fbff",
        loc="left",
        fontsize=14,
        fontweight="bold",
