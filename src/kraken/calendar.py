...
from .models import ExchangeSession, IntegrityIssue, MarketDataQualityPolicy, MarketDataQualityReport, Observation, OptionQuote
...
def _issue(policy: MarketDataQualityPolicy, code: str, message: str) -> IntegrityIssue:
    return IntegrityIssue(code=code, severity="error" if policy.strict else "warning", message=message)
    ...
    for row in rows:
        ...
        try:
            session = ExchangeSession(
                ...
                is_half_day=str(row.get("is_half_day", "false")).lower() in {"1", "true", "yes"},
            )
        except (KeyError, TypeError, ValueError) as exc:
            raise ValueError("Exchange calendar sessions require session_date, open_ms, and close_ms") from exc
        if session.open_ms >= session.close_ms:
            raise ValueError(f"Exchange calendar session has non-positive duration: {session.session_date}")
...
def validate_market_data_quality(
    ...
    for observation in observations:
        ...
        if session is None:
            issues.append(_issue(active_policy, "calendar_session_missing", f"Observation timestamp {observation.timestamp_ms} is not on an explicit exchange session"))
        elif not session.open_ms <= observation.timestamp_ms <= session.close_ms:
            issues.append(_issue(active_policy, "outside_exchange_session", f"Observation timestamp {observation.timestamp_ms} falls outside the declared {date} trading session"))
        key = (observation.timestamp_ms, observation.available_at_ms)
        if key in observation_keys:
            duplicate_snapshot_count += 1
            issues.append(_issue(active_policy, "duplicate_observation_snapshot", f"Duplicate observation snapshot at timestamp {observation.timestamp_ms}"))
        ...
        if lag > active_policy.max_snapshot_lag_ms:
            ...
            issues.append(_issue(active_policy, "stale_observation_snapshot", f"Observation at {observation.timestamp_ms} was published {lag} ms after its timestamp"))
    ordered_dates = tuple(sorted(observed_dates))
    if len(ordered_dates) > 1:
        expected_dates = tuple(item.session_date for item in sessions if ordered_dates[0] <= item.session_date <= ordered_dates[-1])
        missing = tuple(date for date in expected_dates if date not in observed_dates)
        if missing:
            warnings.append(f"Observation history omits {len(missing)} explicit exchange session(s) between its first and last observed date")
    ...
    for quote in option_quotes:
        ...
        session = session_by_date.get(date)
        if session is None:
            issues.append(_issue(active_policy, "option_calendar_session_missing", f"Option quote {quote.contract} is not on an explicit exchange session"))
        elif not session.open_ms <= quote.timestamp_ms <= session.close_ms:
            issues.append(_issue(active_policy, "option_outside_exchange_session", f"Option quote {quote.contract} falls outside the declared {date} trading session"))
        key = (quote.contract, quote.option_type, quote.timestamp_ms, quote.available_at_ms)
        if key in option_keys:
            duplicate_snapshot_count += 1
            issues.append(_issue(active_policy, "duplicate_option_snapshot", f"Duplicate option snapshot for {quote.contract} at {quote.timestamp_ms}"))
        option_keys.add(key)
        lag = quote.available_at_ms - quote.timestamp_ms
        if lag > active_policy.max_snapshot_lag_ms:
            stale_snapshot_count += 1
            issues.append(_issue(active_policy, "stale_option_snapshot", f"Option quote {quote.contract} was published {lag} ms after its timestamp"))
        if quote.underlying in halted:
            issues.append(_issue(active_policy, "halted_underlying_quote", f"Option quote {quote.contract} references halted underlying {quote.underlying}"))
        chain_quotes[(quote.underlying, quote.timestamp_ms, quote.expiration_ms)].append(quote)
    for key, quotes in chain_quotes.items():
        ...
        if reasons:
            ...
            issues.append(_issue(active_policy, "incomplete_option_chain", f"Option chain {key[0]} at {key[1]} requires {' and '.join(reasons)}"))
    ...
    return MarketDataQualityReport(
        ...
        checked_option_quote_count=sum(1 for quote in option_quotes if quote.available_at_ms <= decision_cutoff_ms),
