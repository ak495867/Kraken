...
class FilePointInTimeVendorAdapter:
    ...
    def load(self) -> LicensedPointInTimeDataset:
        ...
        if manifest.mapping_path:
            ...
            if (
                manifest.normalizer_profile
                and mapping_name != manifest.normalizer_profile
            ):
                raise ValueError(
                    "Vendor manifest normalizer_profile does not match the referenced mapping file name"
                )
        equity = load_observations(manifest.equity_path)
        options = load_option_quotes(manifest.options_path)
        corporate_actions = (
            load_corporate_actions(manifest.corporate_actions_path)
            if manifest.corporate_actions_path
            else ()
        )
        option_adjustments = (
            load_option_adjustments(manifest.option_adjustments_path)
            if manifest.option_adjustments_path
            else ()
        )
        distributions = (
            load_distributions(manifest.distributions_path)
            if manifest.distributions_path
            else ()
        )
        earnings_events = (
            load_earnings_events(manifest.earnings_path)
            if manifest.earnings_path
            else ()
        )
        if (
            equity[-1].timestamp_ms > manifest.snapshot_as_of_ms
            or options[-1].timestamp_ms > manifest.snapshot_as_of_ms
        ):
            raise ValueError(
                "Vendor manifest snapshot_as_of predates the supplied data"
            )
        return LicensedPointInTimeDataset(
            manifest,
            equity,
            options,
            corporate_actions,
